...
- Create a `.env` file from the `.env.template` and populate the variables
  ...
  **If you want to make Twilio call to your phone number:**

  In the browser console, you can easily execute `fetch("/api/call/outbound", { method: "POST" })`, making Twilio call to the `TWILIO_TARGET_PHONE` phone number.

  _Tip_: `WS_SERVER_URL` should be the WebSocket server url connection that will stream the audio back and forth from OpenAI
...
- Make this local server available to Twilio with [ngrok](https://ngrok.com/) or [**localtunnel**](https://theboroer.github.io/localtunnel-www/) (preferable if you have 1 tunnel open already with ngrok and can't open any else), after installing it, run ` lt --port 3000`
